...
sys.path.append('/home/AeonG_Cloud/tests/mgbench')
import helpers
import runners
...
def get_binary(args):
    if args.binary_type == "aeong":
        aeong = runners.Memgraph(args.aeong_binary, args.data_directory, not args.no_properties_on_edges,
                                 memgraph_port=args.port, snapshot_interval_sec=30, memory_limit=0,
    ...
    if args.binary_type == "clockg":
        clockg = runners.Memgraph(args.aeong_binary, args.data_directory, not args.no_properties_on_edges,
                                  memgraph_port=args.port, memory_limit=0, snapshot_interval_sec=0, anchor_num=10,
...
if __name__ == "__main__":
    ...
    client = runners.Client(args.client_binary, args.data_directory, memgraph_port=args.port)
    # create index
    ...
    # print("duration",graph_op_ret[0]['duration'])
    # print("count",graph_op_ret[0]['count'])
