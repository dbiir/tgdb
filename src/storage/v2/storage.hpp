...
//wzy begin: Structure for the cache entry
struct CacheEntry {
  int freq;
  std::tuple<uint64_t,uint64_t,uint64_t> key;
  storage::HistoryVertex* val;
  // CacheEntry():freq(0),key(), val(nullptr){};
  CacheEntry(std::tuple<uint64_t,uint64_t,uint64_t> _key, storage::HistoryVertex* _val, int _freq):freq(_freq),key(_key), val(_val){};
};
//wzy end

    class LFUCache {
    private:
        std::map<std::string, std::vector<std::string>> key2ids;
        std::map<std::tuple<uint64_t,uint64_t,uint64_t>, std::list<CacheEntry>::iterator> key_table;
        std::map<int, std::list<CacheEntry>> freq_table;
        int minFreq, capacity;
    public:
        LFUCache():minFreq(1),capacity(1024*1024*64){}
        LFUCache(int _capacity) :minFreq(1), capacity(_capacity) {}
        void put(std::tuple<uint64_t,uint64_t,uint64_t> key, storage::HistoryVertex* value) {
            if (key_table.find(key) != key_table.end()) {
                ...
                freq_table[++freq].push_front(CacheEntry(key, value, freq));
                ...
                freq_table[1].push_front(CacheEntry(key, value, 1));
                key_table[key] = freq_table[1].begin();
            }
        }

        storage::HistoryVertex* get(std::tuple<uint64_t,uint64_t,uint64_t> key) {
            if (key_table.find(key) != key_table.end()) {
                auto node = key_table[key];
                int freq = node->freq;
                auto val = node->val;
                ...
                freq_table[++freq].push_front(CacheEntry(key,val,freq));
                key_table[key] = freq_table[freq].begin();
                return val;
            }
            return nullptr;
...
class Storage final {
 public:
  ...
  class Accessor final {
   ...
   public:
    ...
    std::optional<std::vector<std::tuple<storage::HistoryEdge*,uint64_t,uint64_t>>> FindHistoryEdge(uint64_t gid,uint64_t c_ts,uint64_t c_te){
      // std::map<uint64_t,std::vector<std::tuple<storage::HistoryEdge*,uint64_t,uint64_t>>> all_edge;
      std::vector<std::tuple<storage::HistoryEdge*,uint64_t,uint64_t>> edges;
      auto key=std::make_tuple(gid,c_ts,c_te);
      if(storage_->all_edge.find(key)!=storage_->all_edge.end()){
        return storage_->all_edge[key];
      }
      return std::nullopt;
    }

    void ClearHistory(){
      storage_->all_vertex.clear();
      // storage_->all_vertex_flag.clear();
      storage_->all_edge_flag.clear();
      storage_->all_edge_flag.clear();
    }
    ...
    std::optional<std::list<storage::HistoryVertex*>> FindHistoryVertex(uint64_t gid,uint64_t c_ts,uint64_t c_te){
      ...
      auto key=std::make_tuple(gid,c_ts,c_te);
      if(storage_->all_vertex.find(key)!=storage_->all_vertex.end()){
        return storage_->all_vertex[key];
      }
      return std::nullopt;
    }

    // wzy begin
    storage::HistoryVertex* FindCachedVertex(uint64_t gid,uint64_t c_ts,uint64_t c_te){
      auto key=std::make_tuple(gid,c_ts,c_te);
      return storage_->cache_vertex.get(key);
    }
    // wzy end
    ...
    // wzy begin cache
    void cacheHistoryVertex(uint64_t gid,storage::HistoryVertex* vertex){
      auto key=std::make_tuple(gid,vertex->tt_ts, vertex->tt_te);
      storage_->cache_vertex.put(key, new HistoryVertex(vertex));
    }

    void saveCacheVertexIndexBlank(uint64_t gid,uint64_t c_ts,uint64_t c_te){
      auto key=std::make_tuple(gid,c_ts,c_te);
      storage_->cache_vertex_index[key]=std::list<std::pair<uint64_t,uint64_t>>();
    }

    void saveCacheVertexIndex(uint64_t gid,uint64_t c_ts,uint64_t c_te,uint64_t tt_ts,uint64_t tt_te){
      auto key=std::make_tuple(gid,c_ts,c_te);
      storage_->cache_vertex_index[key].emplace_back(std::make_pair(tt_ts,tt_te));
    }

    std::optional<std::list<std::pair<uint64_t,uint64_t>>> FindCacheVertexIndex(uint64_t gid,uint64_t c_ts,uint64_t c_te){
      auto key=std::make_tuple(gid,c_ts,c_te);
      if(storage_->cache_vertex_index.find(key)!=storage_->cache_vertex_index.end()){
        return storage_->cache_vertex_index[key];
      }
      return std::nullopt;
    }
    ...
    bool FindHistoryVertexFlag(uint64_t gid,uint64_t c_ts,uint64_t c_te){
      ...
      storage_->all_vertex_flag.WithLock([&](const auto &set) {
          find_flag=set.count(key);
          std::cout << "All elements in the set:\n";
          for (const auto &elem : set) {
              std::cout << "(" << std::get<0>(elem) << ", " << std::get<1>(elem) << ", " << std::get<2>(elem) << ")\n";
          }
      });
      std::cout<<"get info:"<<find_flag<<" "<<gid<<" "<<c_ts<<" "<<c_te<<"\n";
      if(find_flag==0){
        return false;
      }
      return true;
    ...
    void saveHistoryVertexFlag(uint64_t gid,uint64_t c_ts,uint64_t c_te){
      auto key=std::make_tuple(gid,c_ts,c_te);
      auto locked_set = storage_->all_vertex_flag.Lock();
      locked_set->insert(key);
      // storage_->all_vertex_flag->insert(key);
      // storage_->all_vertex_flag.WithLock(
      //   [&](auto &all_vertex_flag_) { all_vertex_flag_.insert(key); });
    }
  ...
  //aeong cloud:use for storing kv into the storage 
  bool MigrateToRocksDBCloudStorage();
 ...
 private:
  ...
  utils::Synchronized<std::set<std::tuple<uint64_t,uint64_t,uint64_t>>, utils::SpinLock> all_vertex_flag;

  //wzy begin cache
  LFUCache cache_vertex;
  std::map<std::tuple<uint64_t,uint64_t,uint64_t>,std::list<std::pair<uint64_t,uint64_t>>> cache_vertex_index;
  //wzy end
  ...
  //aeong cloud
  utils::Scheduler cloud_storage_runner_;
