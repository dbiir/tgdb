...
bool addHistoryVertex2(query::VertexAccessor &current_vertex_,history_delta::historyContext &historyContext_,history_delta::historyContext &historyContext2,TypedValue current_edge,std::list<std::pair<TypedValue,TypedValue>> &history_add_,ExecutionContext &context,bool edge_expand){
  ...
  storage::HistoryVertex current_vertex1;
  bool history_flag=false;
  auto [dead_deltas,need_deleted_flag]=history_delta::getDeadInfo2(current_vertex_,historyContext2.c_ts, historyContext2.c_te,historyContext_.types);
  for (auto dead_delta:dead_deltas){
    current_vertex1=context.db_accessor->CreateHistoryVertexFromDelta((current_vertex_).impl_,dead_delta,historyContext_);
    history_flag=true;
    auto values=TypedValue(current_vertex1);
  ...
  //delete info
  auto [gid_history_deltas_,flag]=context.db_accessor->GetHistoryDelta()->GetVertexInfo(current_vertex_.Gid(),historyContext2.c_ts,historyContext2.c_te,historyContext_.types);
  for(auto gid_delta_:gid_history_deltas_){
    if(history_flag){
      current_vertex1=context.db_accessor->CreateHistoryVertexFromKV(current_vertex1,gid_delta_,historyContext_);
    }else {
...
bool addHistoryCachedVertex(query::VertexAccessor &current_vertex_,history_delta::historyContext &historyContext_,std::list<TypedValue> &history_add_,ExecutionContext &context,bool edge_expand){
  auto gid=current_vertex_.Gid().AsUint();
  auto obj_ts=current_vertex_.transaction_st();
  auto obj_te=current_vertex_.tt_te();
  bool delete_flag=false;
  ...
  storage::HistoryVertex current_vertex1;
  bool history_flag=false;
  auto vertexIndex=context.db_accessor->FindCacheVertexIndex(gid,historyContext_.c_ts,historyContext_.c_te);
  // bool findFlag=context.db_accessor->FindHistoryVertexFlag(gid,historyContext_.c_ts,historyContext_.c_te);
  if(vertexIndex){//vertexIndex
    // std::cout<<"get cached vertex"<<historyContext_.c_ts<<" "<<historyContext_.c_te<<"==========\n";
    for(auto vertex_index:*vertexIndex){
      auto cached_vertex=context.db_accessor->FindCachedVertex(gid,vertex_index.first,vertex_index.second);
      auto values = TypedValue(*cached_vertex);
      history_add_.emplace_back(values);
    }
  }else{
    // std::cout<<"do not get cached vertex"<<historyContext_.c_ts<<" "<<historyContext_.c_te<<"==========\n";
    context.db_accessor->saveCacheVertexIndexBlank(gid,historyContext_.c_ts,historyContext_.c_te);
    // context.db_accessor->saveHistoryVertexFlag(gid,historyContext_.c_ts,historyContext_.c_te);
    // bool findFlag=context.db_accessor->FindHistoryVertexFlag(gid,historyContext_.c_ts,historyContext_.c_te);
    // std::cout<<"find flag here:"<<findFlag<<"\n";
    auto [gid_history_deltas_,flag]=context.db_accessor->GetHistoryDelta()->GetVertexInfo(current_vertex_.Gid(),historyContext_.c_ts,historyContext_.c_te,historyContext_.types);
    for(auto gid_delta_:gid_history_deltas_){
        if(history_flag){
            current_vertex1=context.db_accessor->CreateHistoryVertexFromKV(current_vertex1,gid_delta_,historyContext_);
        ...
        auto values=TypedValue(current_vertex1);
        history_add_.emplace_back(values);
        context.db_accessor->cacheHistoryVertex(gid,&current_vertex1);
        context.db_accessor->saveCacheVertexIndex(gid,historyContext_.c_ts,historyContext_.c_te,current_vertex1.tt_ts,current_vertex1.tt_te);
    }
  }
  return delete_flag;
}

bool addHistoryCachedVertex2(query::VertexAccessor &current_vertex_,history_delta::historyContext &historyContext_,history_delta::historyContext &historyContext2,TypedValue current_edge,std::list<std::pair<TypedValue,TypedValue>> &history_add_,ExecutionContext &context,bool edge_expand){
  auto gid=current_vertex_.Gid().AsUint();
  auto obj_ts=current_vertex_.transaction_st();
  auto obj_te=current_vertex_.tt_te();
  bool delete_flag=false;
  ...
  storage::HistoryVertex current_vertex1;
  bool history_flag=false;
  auto vertexIndex=context.db_accessor->FindCacheVertexIndex(gid,historyContext2.c_ts,historyContext2.c_te);
  if(vertexIndex){
    for(auto vertex_index:*vertexIndex){
      auto cached_vertex=context.db_accessor->FindCachedVertex(gid,vertex_index.first,vertex_index.second);
      auto vertex1 = *cached_vertex;
      auto values = TypedValue(vertex1);
      history_add_.emplace_back(current_edge,values); 
    }
  }else{
    context.db_accessor->saveCacheVertexIndexBlank(gid,historyContext_.c_ts,historyContext_.c_te);
    ...
    for(auto gid_delta_:gid_history_deltas_){
        if(history_flag){
            current_vertex1=context.db_accessor->CreateHistoryVertexFromKV(current_vertex1,gid_delta_,historyContext2);
        }else {
            current_vertex1=context.db_accessor->CreateHistoryVertexFromKV((current_vertex_).impl_,gid_delta_,historyContext2);
        }
        history_flag=true;
        auto values=TypedValue(current_vertex1);
        history_add_.emplace_back(current_edge,values); 
        context.db_accessor->cacheHistoryVertex(gid,&current_vertex1);
        context.db_accessor->saveCacheVertexIndex(gid,historyContext2.c_ts,historyContext2.c_te,current_vertex1.tt_ts,current_vertex1.tt_te);
    }
  }
  return delete_flag;
}
  ...
  bool Pull(Frame &frame, ExecutionContext &context) override {
    ...
    if(context.addition){
      if(count==0){
        ...
        historyContext_.c_ts=ts;
        historyContext_.c_te=te;
      ...
      while(true){
        ...
        addHistoryCachedVertex(current_vertex,historyContext_,history_add,context,false);
...
void pull_nodes_current_history(ExecutionContext &context,VertexAccessor current_vertex,uint64_t obj_ts,uint64_t obj_te,TypedValue current_edge,std::list<std::pair<TypedValue,TypedValue>> &history_add_,history_delta::historyContext &historyContext_) {
  ...
  addHistoryCachedVertex2(current_vertex,historyContext_,historyContext2,current_edge,history_add_,context,true); //gmark
  return;
};
