...
// '/home/AeonG_Cloud/src/query/plan/operator.lcp'
...
MakeUniqueCursorPtr(utils::Allocator<TCursor> allocator, TArgs &&...args) {
...
using LogicalOperatorCompositeVisitor = ::utils::CompositeVisitor<
    ...
    ScanAllByLabelProperty, ScanAllById, Expand, ExpandVariable,
    ConstructNamedPath, Filter, Produce, Delete, SetProperty, SetProperties,
    SetLabels, RemoveProperty, RemoveLabels, EdgeUniquenessFilter, Accumulate,
    Aggregate, Skip, Limit, OrderBy, Merge, Optional, Unwind, Distinct, Union,
    Cartesian, CallProcedure, LoadCsv>;
...
class Once : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
...
using PropertiesMapList =
    std::vector<std::pair<storage::PropertyId, Expression *>>;
...
struct NodeCreationInfo {
  ...
  NodeCreationInfo(
      ...
      : symbol{std::move(symbol)}, labels{std::move(labels)},
      ...
      for (auto i0 = 0; i0 < props->size(); ++i0) {
        {
          ...
          Expression *second2;
          second2 = (*props)[i0].second ? (*props)[i0].second->Clone(storage)
                                        : nullptr;
          destination_props[i0] =
              std::make_pair(std::move(first1), std::move(second2));
        }
      }
    } else {
      object.properties =
          std::get<ParameterLookup *>(properties)->Clone(storage);
    }
    return object;
  }
};
...
///
/// @sa CreateExpand
class CreateNode : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  query::plan::NodeCreationInfo node_info_;
...
struct EdgeCreationInfo {
  ...
  EdgeCreationInfo(
      ...
      : symbol{std::move(symbol)}, properties{std::move(properties)},
  ...
  EdgeCreationInfo(Symbol symbol, ParameterLookup *properties,
                   storage::EdgeTypeId edge_type, EdgeAtom::Direction direction)
      : symbol{std::move(symbol)}, properties{properties}, edge_type{edge_type},
        direction{direction} {};
...
///
/// @sa CreateNode
class CreateExpand : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  query::plan::NodeCreationInfo node_info_;
  query::plan::EdgeCreationInfo edge_info_;
  std::shared_ptr<query::plan::LogicalOperator> input_;
  Symbol input_symbol_;
  /// if the given node atom refers to an existing node (either matched or
  /// created)
  bool existing_node_;
...
class ScanAll : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  /// command. With @c storage::View::NEW, all vertices will be produced the
  /// current transaction sees along with their modifications.
  storage::View view_;
...
/// @sa ScanAllByLabelPropertyRange
/// @sa ScanAllByLabelPropertyValue
class ScanAllByLabel : public query::plan::ScanAll {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
/// @sa ScanAllByLabel
/// @sa ScanAllByLabelPropertyValue
class ScanAllByLabelPropertyRange : public query::plan::ScanAll {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  std::unique_ptr<LogicalOperator> Clone(AstStorage *storage) const override {
    ...
    if (lower_bound_) {
      object->lower_bound_.emplace(utils::Bound<Expression *>(
          lower_bound_->value()->Clone(storage), lower_bound_->type()));
    } else {
      object->lower_bound_ = std::nullopt;
    }
    if (upper_bound_) {
      object->upper_bound_.emplace(utils::Bound<Expression *>(
          upper_bound_->value()->Clone(storage), upper_bound_->type()));
    } else {
...
/// @sa ScanAllByLabel
/// @sa ScanAllByLabelPropertyRange
class ScanAllByLabelPropertyValue : public query::plan::ScanAll {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
/// @sa ScanAllByLabelPropertyRange
/// @sa ScanAllByLabelPropertyValue
class ScanAllByLabelProperty : public query::plan::ScanAll {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
class ScanAllById : public query::plan::ScanAll {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
struct ExpandCommon {
  ...
  /// This is where a TypedValue containing a list of expanded edges will be
  /// stored.
  Symbol edge_symbol;
  ...
  /// to expand. If empty, all edges are valid. If not empty, only edges with
  /// one of the given types are valid.
...
/// pulled.
class Expand : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  private:
    using InEdgeT = std::remove_reference_t<
        decltype(*std::declval<VertexAccessor>().InEdges(storage::View::OLD))>;
    using InEdgeIteratorT = decltype(std::declval<InEdgeT>().begin());
    using OutEdgeT = std::remove_reference_t<
    ...
    void InitHistoryEdgesByCurrentVertex(Frame &frame,
                                         ...
                                         TypedValue &);
    void InitHistoryEdgesByHistoryVertex(Frame &frame,
                                         ExecutionContext &context,
                                         TypedValue &vertex_value);
    std::optional<
        std::list<std::pair<storage::HistoryEdge *, storage::HistoryVertex *>>>
        in_history_edges_;
    std::optional<
        std::list<std::pair<storage::HistoryEdge *, storage::HistoryVertex *>>>
        out_history_edges_;
  ...
  std::shared_ptr<query::plan::LogicalOperator> input_;
  Symbol input_symbol_;
  query::plan::ExpandCommon common_;
...
/// pulled.
class ExpandVariable : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  /// input_symbol.
  bool is_reverse_;
  /// Optional lower bound of the variable length expansion, defaults are (1,
  /// inf)
  Expression *lower_bound_;
  /// Optional upper bound of the variable length expansion, defaults are (1,
  /// inf)
  Expression *upper_bound_;
  query::plan::ExpansionLambda filter_lambda_;
  std::optional<query::plan::ExpansionLambda> weight_lambda_;
  ...
  std::unique_ptr<LogicalOperator> Clone(AstStorage *storage) const override {
    ...
    if (weight_lambda_) {
      query::plan::ExpansionLambda value0;
...
class ConstructNamedPath : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  ConstructNamedPath(const std::shared_ptr<LogicalOperator> &input,
      ...
      : input_(input), path_symbol_(path_symbol),
...
/// a boolean value.
class Filter : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
class Produce : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  std::unique_ptr<LogicalOperator> Clone(AstStorage *storage) const override {
    ...
    for (auto i1 = 0; i1 < named_expressions_.size(); ++i1) {
      object->named_expressions_[i1] =
          named_expressions_[i1] ? named_expressions_[i1]->Clone(storage)
                                 : nullptr;
    }
    return object;
  }
...
class Delete : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  std::unique_ptr<LogicalOperator> Clone(AstStorage *storage) const override {
    ...
    for (auto i2 = 0; i2 < expressions_.size(); ++i2) {
      object->expressions_[i2] =
          expressions_[i2] ? expressions_[i2]->Clone(storage) : nullptr;
    }
...
class SetProperty : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
/// updating.
class SetProperties : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  /// means that the old properties are discarded and replaced with new ones.
  ...
  Symbol input_symbol_;
  Expression *rhs_;
  query::plan::SetProperties::Op op_;
...
class SetLabels : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
class RemoveProperty : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
class RemoveLabels : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
/// value (the latest expansion) is not equal to any of the previous_symbols
/// frame values.
///
...
/// Cyphermorphism is edge-uniqueness (the above explanation applies). By
/// default Neo4j uses Cyphermorphism (that's where the name stems from, it is
/// not a valid graph-theory term).
///
/// Supports variable-length-edges (uniqueness comparisons between edges and an
/// edge lists).
class EdgeUniquenessFilter : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
///
/// On the first Pull from this operator's Cursor the input Cursor will be
/// Pulled until it is empty. The results will be accumulated in the temporary
/// cache. Once the input Cursor is empty, this operator's Cursor will start
/// returning cached stuff from its Pull.
///
...
class Accumulate : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
class Aggregate : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }

  /// An aggregation element, contains:
  ///        (input data expression, key expression - only used in COLLECT_MAP,
  ///        type of aggregation, output symbol).
  struct Element {
    static const utils::TypeInfo kType;
    const utils::TypeInfo &GetTypeInfo() const { return kType; }
  ...
  std::shared_ptr<query::plan::LogicalOperator> input_;
  std::vector<query::plan::Aggregate::Element> aggregations_;
  ...
  std::unique_ptr<LogicalOperator> Clone(AstStorage *storage) const override {
    ...
    for (auto i4 = 0; i4 < group_by_.size(); ++i4) {
      object->group_by_[i4] =
          group_by_[i4] ? group_by_[i4]->Clone(storage) : nullptr;
    }
...
class Skip : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
class Limit : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
class OrderBy : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  std::unique_ptr<LogicalOperator> Clone(AstStorage *storage) const override {
    ...
    for (auto i5 = 0; i5 < order_by_.size(); ++i5) {
      object->order_by_[i5] =
          order_by_[i5] ? order_by_[i5]->Clone(storage) : nullptr;
    }
...
/// documentation.
class Merge : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  std::shared_ptr<query::plan::LogicalOperator> input_;
  std::shared_ptr<query::plan::LogicalOperator> merge_match_;
  std::shared_ptr<query::plan::LogicalOperator> merge_create_;

  std::unique_ptr<LogicalOperator> Clone(AstStorage *storage) const override {
    auto object = std::make_unique<Merge>();
    object->input_ = input_ ? input_->Clone(storage) : nullptr;
    object->merge_match_ =
        merge_match_ ? merge_match_->Clone(storage) : nullptr;
    object->merge_create_ =
        merge_create_ ? merge_create_->Clone(storage) : nullptr;
    return object;
  }
...
class Optional : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  std::shared_ptr<query::plan::LogicalOperator> input_;
  std::shared_ptr<query::plan::LogicalOperator> optional_;
...
class Unwind : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  std::unique_ptr<LogicalOperator> Clone(AstStorage *storage) const override {
    ...
    object->input_ = input_ ? input_->Clone(storage) : nullptr;
    object->input_expression_ =
        input_expression_ ? input_expression_->Clone(storage) : nullptr;
    object->output_symbol_ = output_symbol_;
    return object;
  }
};
...
class Distinct : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
...
class Union : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  std::shared_ptr<query::plan::LogicalOperator> left_op_;
  std::shared_ptr<query::plan::LogicalOperator> right_op_;
...
class Cartesian : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  Cartesian(const std::shared_ptr<LogicalOperator> &left_op,
            ...
            const std::shared_ptr<LogicalOperator> &right_op,
            const std::vector<Symbol> &right_symbols)
      : left_op_(left_op), left_symbols_(left_symbols), right_op_(right_op),
...
class OutputTable : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  std::vector<Symbol> output_symbols_;
  std::function<std::vector<std::vector<TypedValue>>(Frame *,
                                                     ExecutionContext *)>
      callback_;
...
/// An operator that outputs a table, producing a single row on each pull.
/// This class is different from @c OutputTable in that its callback doesn't
/// fetch all rows at once. Instead, each call of the callback should return a
/// single row of the table.
class OutputTableStream : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  OutputTableStream(std::vector<Symbol> output_symbols,
                    std::function<std::optional<std::vector<TypedValue>>(
                        Frame *, ExecutionContext *)>
                        callback);
...
class CallProcedure : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  std::unique_ptr<LogicalOperator> Clone(AstStorage *storage) const override {
    ...
    for (auto i6 = 0; i6 < arguments_.size(); ++i6) {
      object->arguments_[i6] =
          arguments_[i6] ? arguments_[i6]->Clone(storage) : nullptr;
    }
    object->result_fields_ = result_fields_;
    object->result_symbols_ = result_symbols_;
    object->memory_limit_ =
        memory_limit_ ? memory_limit_->Clone(storage) : nullptr;
...
private:
  inline static utils::Synchronized<std::unordered_map<std::string, int64_t>,
                                    utils::SpinLock>
      procedure_counters_;
};

class LoadCsv : public query::plan::LogicalOperator {
public:
  static const utils::TypeInfo kType;
  const utils::TypeInfo &GetTypeInfo() const override { return kType; }
  ...
  LoadCsv(std::shared_ptr<LogicalOperator> input, Expression *file,
          bool with_header, bool ignore_bad, Expression *delimiter,
          Expression *quote, Symbol row_var);
  bool Accept(HierarchicalLogicalOperatorVisitor &visitor) override;
  UniqueCursorPtr MakeCursor(utils::MemoryResource *) const override;
  std::vector<Symbol> OutputSymbols(const SymbolTable &) const override;
  std::vector<Symbol> ModifiedSymbols(const SymbolTable &) const override;
